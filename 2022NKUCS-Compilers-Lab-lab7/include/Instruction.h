...
public:
    ...
    Instruction *getNext();
    Instruction *getPrev();
    int getType() {return instType;};  //lab6 获取指令类型
...
protected:
    ...
    enum {BINARY, COND, UNCOND, RET, LOAD, STORE, CMP, ALLOCA, CALL, ZEXT};   //lab6 增加类型 CALL ZEXT
};
...
public:
    ...
    void output() const;
    void genMachineCode(AsmBuilder*);
    enum {SUB, ADD, AND, OR, MUL, DIV, MOD};  //lab 6
};
