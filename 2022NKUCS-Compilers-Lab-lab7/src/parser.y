...
FuncDef
    ...
    FuncParams
    {
        ...
        if ($4 == nullptr)
        {
            ...
            if( identifiers -> isFuncRedef($2, ParamsType))
            {
        ...
        {
            if( identifiers -> isFuncRedef($2, ((FuncDefParamsNode*)$4)->getParamsType()))
            {
                fprintf(stderr, "function \"%s\" redefine\n", $2);
                exit(EXIT_FAILURE);
            }
        ...
        if($4!=nullptr)
        {
            ((FunctionType*)(se->getType()))->setparamsType(((FuncDefParamsNode*)$4)->getParamsType());
        }
    ...
    BlockStmt
    {
        ...
        assert(se != nullptr);
        $$ = new FunctionDef(se,(FuncDefParamsNode*)$4,$7);
        SymbolTable *top = identifiers;
        identifiers = identifiers->getPrev();
        delete top;
        delete []$2;
