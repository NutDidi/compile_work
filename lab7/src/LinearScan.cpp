...
{
    // Todo
    fprintf(stderr,"LinearScan::linearScanRegisterAllocation()\n");
    bool ret = true;
...
{
    fprintf(stderr,"LinearScan::modifyCode()\n");
    for (auto &interval : intervals)
    {
...
{
    fprintf(stderr,"LinearScan::genSpillCode()\n");
    for(auto &interval:intervals)
    {
        ...
        */
        ...
        for (auto use : interval->uses) 
        {
            auto ldr = new LoadMInstruction(use->getParent()->getParent(), use, fp, off);
            use->getParent()->getParent()->insertBefore(use->getParent(),ldr);
            
        }
        for (auto def : interval->defs) 
        {
            auto str = new StoreMInstruction(def->getParent()->getParent(), def, fp, off);
            def->getParent()->getParent()->insertAfter(def->getParent(),str);
        }
    }
}
//modify bd finish 不知道对不对
void LinearScan::expireOldIntervals(Interval *interval)
{
    ...
    */
    ...
    //zhz
    //    auto a = active.begin();
    //    while((*a)->end < interval->start)
    //    {
    //         regs.push_back((*a)->rreg);
    //         a = active.erase(find(active.begin(), active.end(), *a));
    //         //quest??需要排序吗
    //         sort(regs.begin(), regs.end());
    //    }

    //bd
//     fprintf(stderr,"LinearScan::expireOldIntervals(Interval *interval)\n");
//     Interval* x;
//    for(auto a : active)
//    {
...
//             }
//     }
//     active.erase(active.begin(),iter);
    //quest 要不要给reg排序？？？、
    // sort(regs.begin(), regs.end());
...
//modify bd finish 有一个小细节不确定，不知道需不需要把active的最后一个pop了
